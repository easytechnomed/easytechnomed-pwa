...
export async function generateReportPdfOffline(registrationId, options = {}) {
  ...
  if (!reg) {
    throw new Error(`Registration record not found in IndexedDB for: ${registrationId}`);
  }

  // 2. Fetch reference data from IndexedDB
  const [cachedPdfs, doctors, allTests, allTestParams, allParams, patientResultsById, patientResultsByRegNo, allRegTests] =
    await Promise.all([
      ...
      reg.regNo ? db.patientResults.where({ registrationId: reg.regNo }).toArray().catch(() => []) : Promise.resolve([]),
      db.registrationTests.toArray().catch(() => []),
    ]);

  const workspacePdf =
    cachedPdfs.find((wp) => wp.workspaceId === reg.workspaceId) || cachedPdfs[0] || {};
  ...
  const rawRegTests =
    Array.isArray(reg.tests) && reg.tests.length > 0 ? reg.tests : matchingRegTests;
  ...
  const existingResults =
    Array.isArray(reg.results) && reg.results.length > 0
      ? reg.results
      : [...patientResultsById, ...patientResultsByRegNo];
  ...
  if (withFrame) {
    if (framePdfBytes && framePdfBytes.length > 0) {
      ...
      try {
        const binaryString = atob(workspacePdf.framePdfBase64);
        ...
        console.warn("[offlinePdf] Failed to decode framePdfBase64:", e);
      }
